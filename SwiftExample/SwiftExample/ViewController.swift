...
class ViewController: UIViewController {
    ...
    @IBAction func getCredentialFromLib(_ sender: UIButton) {
        Task {
            do {
                if let requestCredential = try await VCIClient(traceabilityId: "My swift app").getCredential(issuerMeta: issuer, signer: signer, accessToken: myAccessToken, publicKey: publicKey) {
